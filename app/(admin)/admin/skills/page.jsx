...
const page = () => {
  ...
  const deleteItem = async (id) => {
    ...
    const res = await axios.delete(`/api/skills?id=${id}`, {
      headers: { "Cache-Control": "no-store" },
    });
            ...
            >
              {item.logopath ? (
                <img
                  src={`/${item.logopath}`}
                  alt="thumb-IMG"
                  className="w-full min-h-[150px] max-h-[150px] bg-black object-contain rounded-md"
                />
              ) : (
              ...
              <div className="flex flex-col gap-2">
                <h2 className="text-lg font-semibold">{item.skillname}</h2>
                <Badge className="text-sm text-gray-400">{item.category}</Badge>
              </div>
              <div className="flex justify-between gap-2 items-center">
                <Link
                  href={`/admin/skills/edit-skills/${item.id}`}
